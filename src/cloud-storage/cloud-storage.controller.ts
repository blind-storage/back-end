...
export class CloudStorageController {
  ...
  @Get('browse')
  @ApiOperation({
    summary: "Contenu d'un dossier : sous-dossiers + fichiers + fil d'Ariane",
  })
  @ApiResponse({
    status: 200,
    description: '{ folder, breadcrumb, folders, files }',
  })
  @ApiResponse({ status: 401, description: 'Non authentifié' })
  ...
  @Get('shared-with-me')
  @ApiOperation({
    summary: "Lister les fichiers partagés avec l'utilisateur connecté",
  })
  ...
  async createFolder(
    @Req() req: any,
    @Body()
    body: { name: string; parentId?: string | null; provider?: CloudProvider },
  ) {
    const userId = req.user.id;
    return this.cloudStorageService.createFolder(
      userId,
      body.name,
      body.parentId ?? null,
      body.provider ?? 'google-drive',
    );
  }

  @Patch('folders/:id')
  @ApiOperation({
    summary: 'Renommer (name) et/ou déplacer (parentId) un dossier',
  })
  ...
  @ApiOperation({
    summary: 'Supprimer un dossier et tout son contenu (récursif)',
  })
  @ApiParam({ name: 'id', description: 'UUID du dossier' })
    ...
    schema: {
      ...
      properties: {
        ...
        enc_fek: {
          type: 'string',
          description:
            "Clé de chiffrement du fichier chiffrée pour l'utilisateur",
        },
        signature: {
          type: 'string',
          description: 'Signature ECDSA du blob chiffré',
          nullable: true,
        },
        replaceFileId: {
          type: 'string',
          nullable: true,
          description: 'Fichier existant à remplacer si conflit de nom',
        },
        replaceMode: {
          type: 'string',
        ...
        replacementShares: {
          type: 'string',
          nullable: true,
          description: 'JSON des enc_fek des destinataires conservés',
        },
        folderId: {
          type: 'string',
          nullable: true,
          description: 'Dossier de destination (vide = racine)',
        },
      },
    },
  })
  @ApiResponse({
    status: 201,
    description: 'Fichier uploadé, retourne le fileId DB',
  })
  @ApiResponse({ status: 401, description: 'Non authentifié' })
  ...
  @ApiOperation({
    summary: 'Télécharger un fichier par son ID base de données',
  })
  @ApiParam({
    name: 'fileId',
    description: 'UUID du fichier en base de données',
  })
  ...
  @Patch('files/:fileId')
  @ApiOperation({
    summary: 'Renommer (name) et/ou déplacer (folderId) un fichier',
  })
  @ApiParam({
    name: 'fileId',
    description: 'UUID du fichier en base de données',
  ...
  @ApiOperation({
    summary: 'Lister les utilisateurs avec qui le fichier est partagé',
  })
  @ApiParam({
    name: 'fileId',
    description: 'UUID du fichier en base de données',
  ...
  async listFileShares(@Param('fileId') fileId: string, @Req() req: any) {
    const userId = req.user.id;
    const shares = await this.cloudStorageService.listFileShares(
      userId,
      fileId,
    );
    return { shares };
  ...
  @ApiBody({
    schema: {
      ...
      properties: {
        ...
        enc_fek: {
          type: 'string',
          description: 'FEK chiffrée avec la clé publique du destinataire',
        },
        ...
        manage: {
          type: 'boolean',
          default: false,
          description: 'Autorise la gestion des partages du fichier',
        },
      },
    },
  })
  ...
  @ApiResponse({
    status: 404,
    description: 'Fichier ou utilisateur introuvable',
  })
  async shareFile(
    @Param('fileId') fileId: string,
    @Req() req: any,
  ...
  async revokeFileShare(
    ...
    const userId = req.user.id;
    await this.cloudStorageService.revokeFileShare(
      userId,
      fileId,
      recipientUserId,
    );
  ...
  @Patch('files/:fileId/shares/:userId')
  @ApiOperation({ summary: "Modifier les droits d'un partage existant" })
  @ApiParam({
    name: 'fileId',
    description: 'UUID du fichier en base de données',
  })
  ...
  async updateFileShare(
    ...
    @Param('userId') recipientUserId: string,
    @Req() req: any,
    @Body() body: { read?: boolean; write?: boolean; manage?: boolean },
  ) {
    const userId = req.user.id;
    const share = await this.cloudStorageService.updateFileShare(
      userId,
      fileId,
      recipientUserId,
      body,
  ...
  @Get('providers')
  @ApiOperation({
    summary: "Statut de connexion des stockages de l'utilisateur",
  })
  @ApiResponse({
    status: 200,
    description: '{ "google-drive": { connected }, "dropbox": { connected } }',
  })
  @ApiResponse({ status: 401, description: 'Non authentifié' })
  async providersStatus(@Req() req: any) {
    return this.cloudStorageService.getProvidersStatus(req.user.id);
  }

  @Get(':provider/connect')
  @ApiOperation({
    summary:
      'URL de consentement OAuth pour connecter un stockage (le client y redirige le navigateur)',
  })
